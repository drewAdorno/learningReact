...
const idList= pokemon.filter(p => p.id%3===0)
// console.log(idList);
...
// console.log(names);
const greaterThan99=pokemon.filter(p => p.id >99).map(p => p.name)
// console.log(greaterThan99);
const poison =  pokemon.filter(p => p.types.length ==1 && p.types[0] ==='poison').map(p => p.name);
// console.log(poison);
const flying = pokemon.filter(p => p.types[1] ==="flying").map(p => p.types[0]);
// console.log(flying);
const num = pokemon.filter(p => p['types'].includes('normal')).length;
console.log(num);
